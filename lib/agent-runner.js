...
class AgentRunner {
  ...
  async describePiModels() {
    if (this.selected === 'pi' && this.worker) {
      return {
        current: this.worker.status().model,
        preferred: this.piPreferredModel,
        available: await this.worker.availableModels(),
      };
    }
    return { current: null, preferred: this.piPreferredModel, available: [] };
  }
