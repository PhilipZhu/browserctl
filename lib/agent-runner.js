  ...
  async ensureWorker(onOutput) {
    ...
    try {
      ...
      if (this.selected === 'pi' && this.piPreferredModel) {
        ...
        if (current?.provider !== this.piPreferredModel.provider ||
            current?.id !== this.piPreferredModel.id) {
          const selected = await worker.setModel(this.piPreferredModel);
          onOutput(`Using previously responsive Pi model ${selected.id} for this console.\n`);
        }
      }
      this.worker = worker;
      this.workerFallback = null;
