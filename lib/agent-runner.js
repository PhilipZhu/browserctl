...
// Tolerant extraction of the completion-audit verdict. Returns null when no
// well-formed verdict is present; callers treat null as "assume complete" so a
// malformed audit can never block a finished turn.
function parseCompletionVerdict(text) {
  const raw = String(text || '').trim();
  const candidates = [];
  const fenced = raw.match(/```(?:json)?\s*([\s\S]*?)```/);
  if (fenced) candidates.push(fenced[1].trim());
  candidates.push(raw);
  candidates.push(...(raw.match(/\{[\s\S]*?\}/g) || []));
  for (const candidate of candidates) {
    try {
      const parsed = JSON.parse(candidate);
      if (parsed && typeof parsed === 'object' && typeof parsed.complete === 'boolean') {
        return {complete: parsed.complete, remainder: String(parsed.remainder || '').trim()};
      }
    } catch {
...
// Two consecutive identical proposals are a loop, not intent: the second is
// refused so a confused agent cannot re-apply the same mutation forever.
function semanticActionKey(proposal) {
...
function completionAuditPrompt(userPrompt, actions, finalOutput) {
  const actionLines = actions.map((action, index) =>
    `${index + 1}. ${action.capability}` +
    `${action.target ? ` → ${action.target}` : ''}` +
    `${action.operation ? ` (${action.operation})` : ''}: ` +
    truncate(String(action.output || '(no output)'), 400)).join('\n');
  return 'You are auditing whether a human request was fully addressed. Do not perform any work and do not use tools.\n\n' +
    `Human request:\n${userPrompt}\n\n` +
    `Verified application actions executed:\n${actionLines}\n\n` +
    `Agent's closing summary:\n${truncate(String(finalOutput || '(none)'), 1200)}\n\n` +
    'Respond with ONLY this JSON object and nothing else:\n' +
    '{"complete": true|false, "remainder": "concise description of any unaddressed part, or empty string"}\n' +
    'Mark complete=false only when a concrete requested outcome was not performed.';
}
...
class AgentRunner {
  constructor(session, store, options = {}) {
    ...
    // Loud runaway backstop, never a normal stop: completion is declared by the
    // agent, and hitting this limit is reported explicitly to the human.
    this.semanticActionLimit = Number.isInteger(options.semanticActionLimit) &&
      options.semanticActionLimit > 0 ? options.semanticActionLimit : 25;
    this.activeSemanticTurn = null;
    this.currentRequestActions = [];
  ...
  async run(userPrompt, onOutput = (chunk) => process.stdout.write(chunk), options = {}) {
    ...
    if (options.record !== false && !options.semanticChain && !options.skipCapabilities) {
      this.currentRequestActions = [];
      this.lastSemanticActionKey = null;
    }
    const failures = [];
    ...
    while (true) {
      try {
        ...
        return await this.finishRequest(userPrompt, output, onOutput, options);
      } catch (error) {
  ...
  // After a turn that executed application actions, one bounded model pass
  // compares the request against what verifiably ran. A found remainder is
  // reported loudly and gets exactly one follow-up turn; audit failures never
  // block a finished turn.
  async finishRequest(userPrompt, output, onOutput, options = {}) {
    if (
      options.record === false ||
      options.skipCapabilities ||
      options.semanticChain ||
      options.recoveryTurn ||
      !this.currentRequestActions.length
    ) {
      return output;
    }
    let verdict = null;
    try {
      const raw = await this.runOnce(
        completionAuditPrompt(userPrompt, this.currentRequestActions, output),
        () => {},
        {
      ...
      verdict = parseCompletionVerdict(raw);
    } catch {
      return output;
    }
    if (!verdict || verdict.complete !== false || !verdict.remainder) return output;
    onOutput(
      `\u001b[33mCompletion audit: part of the request appears unaddressed — ${verdict.remainder}\u001b[0m\n` +
      'Starting one follow-up turn for the remainder.\n',
    );
    try {
      const followUp = await this.run(
        `The original request was:\n${userPrompt}\n\nA completion audit found this part unaddressed: ${verdict.remainder}\n\nAddress exactly that remainder now, verifying each change. If it is already satisfied or cannot be done safely, say so plainly instead of acting.`,
        onOutput,
        {
          memoryMode: options.memoryMode || this.memoryMode,
          record: false,
          semanticChain: {depth: 1},
        },
      );
      return `${output}\n${String(followUp || '').trim()}`.trim();
    } catch (error) {
      onOutput(
        `\u001b[33mThe follow-up attempt failed: ${error.message}. Still unaddressed: ${verdict.remainder}\u001b[0m\n`,
      );
      return output;
    }
  }
  ...
  async runOnce(userPrompt, onOutput = (chunk) => process.stdout.write(chunk), options = {}) {
    try {
      return await this.runTurnOnce(userPrompt, onOutput, options);
    } finally {
      this.activeSemanticTurn = null;
    }
  }

  // Executes one mid-turn application action on behalf of the live agent (the
  // Pi tool calls back through the browser bridge while its turn is running).
  // Validation, execution, and verification stay in this process; the result
  // or error becomes the agent's tool result so it can keep working.
  async executeSemanticProposal(rawArgs) {
    const turn = this.activeSemanticTurn;
    if (!turn) {
      throw new Error('No live agent turn is accepting application actions right now.');
    }
    if (this.currentRequestActions.length >= this.semanticActionLimit) {
      const message =
        `The ${this.semanticActionLimit}-action runaway backstop for this request is exhausted; ` +
        'no further application actions will run. Summarize what was completed and what remains.';
      turn.onOutput(`\u001b[33m${message}\u001b[0m\n`);
      throw new Error(message);
    }
    const proposal = semanticProposalFromToolArgs(rawArgs);
    const proposalKey = semanticActionKey(proposal);
    if (proposalKey === this.lastSemanticActionKey) {
      throw new Error(
        'This action is identical to the one just performed and was not repeated. ' +
        'Continue with a different step, or summarize what was verifiably done.',
      );
    }
    const plan = await prepareSemanticAction(
      turn.semanticExtensions,
      proposal,
      turn.extensionContext,
    );
    const result = await turn.extensionContext.invokeBrowserHook(
      plan.descriptor.hook,
      plan.payload,
    );
    this.lastSemanticActionKey = proposalKey;
    const record = {
      capability: plan.descriptor.id,
      target: plan.payload.target || null,
      operation: plan.payload.operation || null,
    };
    if (plan.descriptor.continueAfterHook) {
      const activation = String(result?.output || result?.summary || '').trim();
      const continuation = typeof plan.descriptor.continuationPrompt === 'function'
        ? await plan.descriptor.continuationPrompt(result, plan, turn.extensionContext)
        : plan.descriptor.continuationPrompt ||
      ...
      this.currentRequestActions.push({...record, output: activation});
      return {output: [activation, continuation].filter(Boolean).join('\n\n')};
    }
    const formatted = plan.descriptor.formatResult
      ? await plan.descriptor.formatResult(result, plan, turn.extensionContext)
      : result?.output || result?.summary;
    const output = String(formatted ?? '').trim();
    this.currentRequestActions.push({...record, output});
    if (output) turn.onOutput(`${output}\n`);
    return {output};
  }

  async runTurnOnce(userPrompt, onOutput = (chunk) => process.stdout.write(chunk), options = {}) {
    this.cancelled = false;
    const descriptor = AGENTS[this.selected];
    ...
    // Mid-turn application actions arrive through the browser bridge while the
    // agent is still running; this is the context they execute against.
    this.activeSemanticTurn = {extensionContext, onOutput, semanticExtensions};
    ...
    let finalOutput = stripAnsi(result.output).trim();
    const capturedSemanticEntries = (Array.isArray(result.semanticProposals)
      ? result.semanticProposals
      : []).map((entry) => (entry && typeof entry === 'object' && 'proposal' in entry
      ? entry
      : {proposal: entry, executed: false, failed: false}));
    const executedSemanticActions = capturedSemanticEntries.filter((entry) => entry.executed);
    const capturedSemanticProposals = capturedSemanticEntries
      .filter((entry) => !entry.executed && !entry.failed)
    ...
    if (capturedSemanticProposals.length > 1) {
      ...
      throw markFailure(
        new Error(
          `The agent left ${capturedSemanticProposals.length} unexecuted application proposals in one turn; none of those were applied.` +
          (executedSemanticActions.length
            ? ` ${executedSemanticActions.length} earlier action${executedSemanticActions.length === 1 ? ' was' : 's were'} already executed and verified this turn and must not be repeated.`
            : ''),
        ),
        {
          phase: 'semantic-action-validation',
          sideEffects: executedSemanticActions.length
            ? `${executedSemanticActions.length} verified application action(s) already applied this turn; inspect current state and do not repeat them`
            : 'none from semantic application hooks',
        },
      );
    }
    ...
    if (!finalOutput && !semanticProposal && executedSemanticActions.length) {
      const lastAction = executedSemanticActions[executedSemanticActions.length - 1];
      result.output = String(lastAction.output || 'The requested application actions were executed and verified.');
      finalOutput = stripAnsi(result.output).trim();
    } else if (!finalOutput && !semanticProposal) {
      if (verbose) flushDeferredAgentOutput();
      if (memoryMode === 'managed') await this.stopWorker();
    ...
    if (semanticProposal) {
      const legacyProposalKey = semanticActionKey(semanticProposal);
      if (legacyProposalKey === this.lastSemanticActionKey) {
        if (verbose) flushDeferredAgentOutput();
        if (memoryMode === 'managed') await this.stopWorker();
        const notice = 'The proposed application action is identical to the one just performed; it was not repeated.';
        onOutput(`${notice}\n`);
        return notice;
      }
      this.lastSemanticActionKey = legacyProposalKey;
      ...
      this.currentRequestActions.push({
        capability: semanticPlan.descriptor.id,
        target: semanticPlan.payload.target || null,
        operation: semanticPlan.payload.operation || null,
        output: String(semanticOutput || '').trim(),
      });
      if (this.currentRequestActions.length < this.semanticActionLimit) {
        // Hand the verified result back to the agent so it can finish any
        // remaining parts of the request; recursion through this same path
        // chains further actions until the agent stops proposing them.
        onActivity?.({
          type: 'extension',
          extensionId: semanticPlan.extension.id,
          label: semanticPlan.descriptor.label,
        });
        onOutput(`${String(semanticOutput || '').trimEnd()}\n`);
        const chainDepth = (options.semanticChain?.depth || 0) + 1;
        const continuation = await this.run(
          `The proposed application action was validated, executed, and verified. Result:\n${semanticOutput}\n\nOriginal request:\n${userPrompt}\n\nIf parts of the original request remain unaddressed, continue now: inspect state as needed and propose the next single application action. If everything is complete, summarize what was verifiably done without proposing another action.`,
          onOutput,
          {
            memoryMode,
            record: false,
            autoRecover: false,
            semanticChain: {depth: chainDepth},
          },
        );
        const combined = `${String(semanticOutput || '').trim()}\n${String(continuation || '').trim()}`.trim();
        if (memoryMode === 'managed' && options.record !== false && this.conversationStore) {
          await this.conversationStore.appendTurn({
            agent: this.selected,
            user: userPrompt,
            assistant: stripAnsi(combined).trim(),
            usage: null,
          });
        }
        return combined;
      ...
      onOutput(
        `\u001b[33mThe ${this.semanticActionLimit}-action runaway backstop for this request is exhausted; stopping after this action. Anything not yet done remains outstanding.\u001b[0m\n`,
      );
    ...
    const processStructuredLine = (line) => {
      ...
      if (event.type === 'tool_execution_end' &&
        ...
        if (proposal) {
          ...
          semanticProposals.push({
            proposal,
            executed: details?.executed === true,
            failed: Boolean(event.isError || details?.failed),
          });
        }
      }
      if (event.type === 'tool_execution_end' &&
