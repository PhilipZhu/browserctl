...
${piToolProtocol
    ? `When a capability safely expresses the next needed outcome, do not invoke its hook through a shell command and do not mutate first. Call \`${PI_SEMANTIC_TOOL}\` with that one action; browserctl validates and executes it immediately and the verified result (or the exact error) returns as the tool result. Include a brief natural-language \`interpretation\` and contextual \`rationale\` so the human can see what you understood. For a typed-payload capability, encode its input object in \`payloadJson\`. A request may need several actions: after each result, continue with the remaining parts, or inspect and correct course on an error, calling the tool again as needed. Finish with a summary of what was verifiably done and anything left undone.`
    : `When a capability safely expresses the next needed outcome, do not invoke its hook through a shell command and do not mutate first. Return only this JSON proposal as your final response; browserctl executes it and hands the verified result back to you so you can continue with any remaining parts of the request:
